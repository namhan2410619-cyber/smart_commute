...
class RouteAgent:
    ...
    def estimate_walk_time_km(self, km, speed_kmh=4.5):
        return max(1, int((km / speed_kmh) * 60))

    def estimate_bus_time(self, s,e):
        km = self.haversine(s,e)
        return max(1, int((km/25)*60))

    def estimate_subway_time(self, s,e):
        km = self.haversine(s,e)
        return max(1, int((km/40)*60))

    def get_osrm_route_coords(self, start, end, mode="walking"):
        # start/end as (lat,lon)
